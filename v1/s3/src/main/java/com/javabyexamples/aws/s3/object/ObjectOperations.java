...
public class ObjectOperations {
    ...
    private PutObjectResult createObjectWithMetadata(String bucketName, String key) {
        final ObjectMetadata objectMetadata = new ObjectMetadata();
        objectMetadata.addUserMetadata("hello", "world");
        byte[] contentBytes = "Hello AWS!".getBytes(StringUtils.UTF8);
    ...
    private PutObjectResult createObjectWithTags(String bucketName, String key) {
        final ObjectMetadata objectMetadata = new ObjectMetadata();
        objectMetadata.addUserMetadata("hello", "world");
        final ObjectTagging objectTagging = new ObjectTagging(List.of(new Tag("first", "tag")));
    ...
    public void listObjectKeys(String bucketName) {
...
//        objectOperations.deleteVersion(versionedBucket, anotherKey, putObjectResult.getVersionId());
        ...
        objectOperations.listObjectKeys(bucketName);

        System.out.println("Create object with metadata.");
        objectOperations.createObjectWithMetadata(bucketName, UUID.randomUUID().toString());

        System.out.println("Create object with tagging.");
        objectOperations.createObjectWithTags(bucketName, UUID.randomUUID().toString());
    }
}
